 ...
 */
function fashion_lite_theme_setup(){

    /*
     * Make child theme available for translation.
     * Translations can be filed in the /languages/ directory.
     */
     load_child_theme_textdomain( 'fashion-lite', get_stylesheet_directory() . '/languages' );

    add_image_size( 'fashion-lite-slider', 1920, 760, true);
    add_image_size( 'fashion-lite-blog-three', 420, 502, true );
    add_image_size( 'fashion-lite-blog-four', 573, 685, true);
}
add_action( 'after_setup_theme', 'fashion_lite_theme_setup', 100 );
/**
 * Load assets.
 */
function fashion_lite_enqueue_styles() {

    $my_theme = wp_get_theme();
    $version = $my_theme['Version'];
    
    wp_enqueue_style( 'vilva', get_template_directory_uri() . '/style.css' );
    wp_enqueue_style( 'fashion-lite', get_stylesheet_directory_uri() . '/style.css', array( 'vilva' ), $version );


	wp_enqueue_script( 'fashion-lite', get_stylesheet_directory_uri() . '/js/custom.js', array( 'jquery'), $version, true );
    ...
    wp_localize_script( 'fashion-lite', 'fashion_lite_data', $array );
}
add_action( 'wp_enqueue_scripts', 'fashion_lite_enqueue_styles', 10 );

function fashion_lite_remove_parent_filters(){
    remove_action( 'customize_register', 'vilva_customize_register_appearance' );
    remove_action( 'wp_head', 'vilva_dynamic_css', 99 );
}
add_action( 'init', 'fashion_lite_remove_parent_filters' );

function fashion_lite_customize_register( $wp_customize ) {

    $wp_customize->add_section( 'theme_info', array(
        'title'       => __( 'Demo & Documentation' , 'fashion-lite' ),
        'priority'    => 6,
    ) );
    ...
    $theme_info = '<p>';
    $theme_info .= sprintf( __( 'Demo Link: %1$sClick here.%2$s', 'fashion-lite' ),  '<a href="' . esc_url( 'https://blossomthemes.com/theme-demo/?theme=fashion-lite' ) . '" target="_blank">', '</a>' ); 
    $theme_info .= '</p><p>';
    $theme_info .= sprintf( __( 'Documentation Link: %1$sClick here.%2$s', 'fashion-lite' ),  '<a href="' . esc_url( 'https://docs.blossomthemes.com/fashion-lite/' ) . '" target="_blank">', '</a>' ); 
    $theme_info .= '</p>';
    ...
    $wp_customize->add_panel(
        'appearance_settings',
        array(
            'title'      => __( 'Appearance Settings', 'fashion-lite' ),
            'priority'   => 30,
    ...
    $wp_customize->add_control( 
        new WP_Customize_Color_Control( 
            ...
            'primary_color', 
            array(
                'label'       => __( 'Primary Color', 'fashion-lite' ),
                'description' => __( 'Primary color of the theme.', 'fashion-lite' ),
                'section'     => 'colors',
                'priority'    => 5,
            )
        )
    ...
    $wp_customize->add_section(
        'typography_settings',
        array(
            'title'    => __( 'Typography Settings', 'fashion-lite' ),
            'priority' => 20,
    ...
    $wp_customize->add_control(
        new Vilva_Select_Control(
            ...
            'primary_font',
            array(
                'label'       => __( 'Primary Font', 'fashion-lite' ),
                'description' => __( 'Primary font of the site.', 'fashion-lite' ),
    ...
    $wp_customize->add_control(
        new Vilva_Select_Control(
            ...
            'secondary_font',
            array(
                'label'       => __( 'Secondary Font', 'fashion-lite' ),
                'description' => __( 'Secondary font of the site.', 'fashion-lite' ),
                'section'     => 'typography_settings',
                'choices'     => vilva_get_all_fonts(),   
            )
        )
    ...
    $wp_customize->add_control(
        new Vilva_Slider_Control( 
            ...
            array(
                'section'     => 'typography_settings',
                'label'       => __( 'Font Size', 'fashion-lite' ),
                'description' => __( 'Change the font size of your site.', 'fashion-lite' ),
                'choices'     => array(
                    'min'   => 10,
                    'max'   => 50,
                    'step'  => 1,
    ...
    $wp_customize->add_control(
        new Vilva_Toggle_Control( 
            ...
            array(
                'section'       => 'typography_settings',
                'label'         => __( 'Load Google Fonts Locally', 'fashion-lite' ),
                'description'   => __( 'Enable to load google fonts from your own server instead from google\'s CDN. This solves privacy concerns with Google\'s CDN and their sometimes less-than-transparent policies.', 'fashion-lite' )
            )
        )
    );   
    ...
    $wp_customize->add_control(
        new Vilva_Toggle_Control( 
            ...
            array(
                'section'       => 'typography_settings',
                'label'         => __( 'Preload Local Fonts', 'fashion-lite' ),
                'description'   => __( 'Preloading Google fonts will speed up your website speed.', 'fashion-lite' ),
    ...
    ob_start(); ?>
        ...
        <span style="margin-bottom: 5px;display: block;"><?php esc_html_e( 'Click the button to reset the local fonts cache', 'fashion-lite' ); ?></span>
        
        <input type="button" class="button button-primary vilva-flush-local-fonts-button" name="vilva-flush-local-fonts-button" value="<?php esc_attr_e( 'Flush Local Font Files', 'fashion-lite' ); ?>" />
    <?php
    $fashion_lite_flush_button = ob_get_clean();
    ...
    $wp_customize->add_control(
        'ed_flush_local_fonts',
        array(
            'label'         => __( 'Flush Local Fonts Cache', 'fashion-lite' ),
            'section'       => 'typography_settings',
            'description'   => $fashion_lite_flush_button,
            'type'          => 'hidden',
            'active_callback' => 'vilva_ed_localgoogle_fonts'
        )
    );
    ...
    $wp_customize->add_panel(
        ...
        array(
            'priority'    => 45,
            'capability'  => 'edit_theme_options',
            'title'    => __( 'Layout Settings', 'fashion-lite' ),
        )
    );
    ...
    $wp_customize->add_section(
        'header_layout',
        array(
            'title'    => __( 'Header Layout', 'fashion-lite' ),
            'priority' => 20,
    ...
    $wp_customize->add_control(
        new Vilva_Radio_Image_Control(
            ...
            array(
                ...
                'label'       => __( 'Header Layout', 'fashion-lite' ),
                'description' => __( 'Choose the layout of the header for your site.', 'fashion-lite' ),
                'choices'     => array(
                    'one'   => get_stylesheet_directory_uri() . '/images/header/one.jpg',
                    'three'   => get_stylesheet_directory_uri() . '/images/header/two.jpg',
                )
            )
        )
    );
    ...
    $wp_customize->add_section(
        ...
        array(
            'title'    => __( 'Slider Layout', 'fashion-lite' ),
            'priority' => 20,
            'panel'    => 'layout_settings',
        )
    );
    ...
    $wp_customize->add_control(
        new Vilva_Radio_Image_Control(
            ...
            array(
                ...
                'label'       => __( 'Slider Layout', 'fashion-lite' ),
                'description' => __( 'Choose the layout of the slider for your site.', 'fashion-lite' ),
                'choices'     => array(
     ...
     $wp_customize->add_section(
        ...
        array(
            'title'    => __( 'Home Page Layout', 'fashion-lite' ),
            'priority' => 30,
            'panel'    => 'layout_settings',
        )
    );
    ...
    $wp_customize->add_control(
        new Vilva_Radio_Image_Control(
            ...
            array(
                ...
                'label'       => __( 'Homepage Layout', 'fashion-lite' ),
                'description' => __( 'Choose the layout of the homepage for your site.', 'fashion-lite' ),
                'choices'     => array(
    ...
    $wp_customize->add_section(
        ...
        array(
            'title'    => __( 'General Sidebar Layout', 'fashion-lite' ),
            'panel'    => 'layout_settings'
        )
    );
    ...
    $wp_customize->add_control(
        new Vilva_Radio_Image_Control(
            ...
            array(
                ...
                'label'       => __( 'Page Sidebar Layout', 'fashion-lite' ),
                'description' => __( 'This is the general sidebar layout for pages. You can override the sidebar layout for individual page in respective page.', 'fashion-lite' ),
                'choices'     => array(
    ...
    $wp_customize->add_control(
        new Vilva_Radio_Image_Control(
            ...
            array(
                'section'     => 'general_layout_settings',
                'label'       => __( 'Post Sidebar Layout', 'fashion-lite' ),
                'description' => __( 'This is the general sidebar layout for posts & custom post. You can override the sidebar layout for individual post in respective post.', 'fashion-lite' ),
                'choices'     => array(
                    'no-sidebar'    => esc_url( get_template_directory_uri() . '/images/1c.jpg' ),
                    'centered'      => esc_url( get_template_directory_uri() . '/images/1cc.jpg' ),
                    'left-sidebar'  => esc_url( get_template_directory_uri() . '/images/2cl.jpg' ),
    ...
    $wp_customize->add_control(
        new Vilva_Radio_Image_Control(
            ...
            array(
                'section'     => 'general_layout_settings',
                'label'       => __( 'Default Sidebar Layout', 'fashion-lite' ),
                'description' => __( 'This is the general sidebar layout for whole site.', 'fashion-lite' ),
                'choices'     => array(
                    'no-sidebar'    => esc_url( get_template_directory_uri() . '/images/1c.jpg' ),
                    'left-sidebar'  => esc_url( get_template_directory_uri() . '/images/2cl.jpg' ),
                    'right-sidebar' => esc_url( get_template_directory_uri() . '/images/2cr.jpg' ),
...
add_action( 'customize_register', 'fashion_lite_customize_register', 90 );
...
function fashion_lite_search_cart(){ 
...
function vilva_banner(){
    if( is_front_page() || is_home() ) {
        ...
        $banner_title   = get_theme_mod( 'banner_title', __( 'Find Your Best Holiday', 'fashion-lite' ) );
        $banner_subtitle = get_theme_mod( 'banner_subtitle' , __( 'Find great adventure holidays and activities around the planet.', 'fashion-lite' ) ) ;
        $banner_button   = get_theme_mod( 'banner_button', __( 'Read More', 'fashion-lite' ) );
        ...
        if( $slider_layout == 'one' ){
            ...
            $image_size = 'fashion-lite-slider';
        }
    ...
    if( is_home() ){        
        echo '<figure class="post-thumbnail"><a href="' . esc_url( get_permalink() ) . '" class="post-thumbnail">';
            if( $home_layout == 'one' ) {
                ...
                $image_size = ( $sidebar ) ? 'fashion-lite-blog-three' : 'fashion-lite-blog-four';
            }
    ...
    if( ( is_home() || is_archive() || is_search() ) && $home_layout == 'one' ) echo '<div class="content-wrap">';
        ...
        echo '<div class="entry-content" itemprop="text">';
            if( is_singular() || ! $ed_excerpt || ( get_post_format() != false ) ){
                the_content();    
                wp_link_pages( array(
                    'before' => '<div class="page-links">' . esc_html__( 'Pages:', 'fashion-lite' ),
                    'after'  => '</div>',
                ) );
            }else{
                the_excerpt();
...
function vilva_fonts_url(){
    ...
    */
    $primary    = _x( 'on', 'Primary Font: on or off', 'fashion-lite' );
    $secondary  = _x( 'on', 'Secondary Font: on or off', 'fashion-lite' );
    $site_title = _x( 'on', 'Site Title Font: on or off', 'fashion-lite' );
 ...
 * Dynamic CSS
 */   
function fashion_lite_dynamic_css(){
    ...
    $primary_color    = get_theme_mod( 'primary_color', '#e18284' );
...
add_action( 'wp_head', 'fashion_lite_dynamic_css', 99 );
