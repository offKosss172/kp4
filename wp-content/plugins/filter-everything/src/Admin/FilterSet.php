            ...
            $l10n = array(
                ...
                'numFieldAttrs'      => [
                    'post_meta_num' => [
                        ...
                        'description'   => esc_html__( 'Name of the Custom Field. Please, see Popular Meta keys at the bottom', 'filter-everything' ),
    ...
    public function addCustomLabel( $html, $attributes )
    {
        if( isset( $attributes['id'] ) ){
            ...
            if( $attributes['id'] == $this->generateFieldId('wp_page_type') ){
                $label          = '<label class="wpc-location-label" for="'.esc_attr($attributes['id']).'">'.esc_html__( 'Apply filtering if page is:', 'filter-everything' ).'</label>'."\r\n";
                $html =  $label . $html;
            }
